...
`kenpachi` automatically maps tool Zod schemas to provider-specific function declarations:

- **Anthropic**: Maps tools to `{ name, description, input_schema: { type: "object", properties, required } }`.
- **OpenAI**: Maps tools to `{ type: "function", function: { name, description, parameters: { type: "object", properties, required } } }`.